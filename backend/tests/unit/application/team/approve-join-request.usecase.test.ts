...
describe('approveJoinRequest', () => {
  it('요청이 없으면 NotFoundError', async () => {
    ...
    await expect(
      approveJoinRequest(repo, { joinRequestId: 'missing', actorUserId: 'leader-1' }),
  ...
  it('팀장이 아니면 승인할 수 없다', async () => {
    ...
    await expect(
      approveJoinRequest(repo, { joinRequestId: REQUEST.id, actorUserId: 'user-3' }),
  ...
  it('팀장이 승인하면 트랜잭션 기반 MEMBER 생성 결과를 반환한다', async () => {
    ...
    await expect(
      approveJoinRequest(repo, { joinRequestId: REQUEST.id, actorUserId: 'leader-1' }),
