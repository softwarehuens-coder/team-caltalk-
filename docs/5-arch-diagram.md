...
| 근거 문서 | [1-domain-definition.md](./1-domain-definition.md) (v1.4) — 도메인 용어, 액터/권한 SSOT<br>[2-PRD.md](./2-PRD.md) — MVP 범위<br>[3-User-scenarios.md](./3-User-scenarios.md) — 실사용 흐름<br>[4-project-structure.md](./4-project-structure.md) (v1.1) — 확정 스택(FE/BE/WebSocket/DB), 레이어 구조 (본 다이어그램의 1차 근거)<br>[6-tech-stack.md](./6-tech-stack.md) (v1.0) — DB(PostgreSQL) 비교·결정 근거 |
...
graph LR
    ...
    subgraph BE["백엔드 (Node.js + TypeScript)"]
        Entry["진입점<br/>(REST 라우트 / WebSocket 게이트웨이)<br/>— 인증 강제"]
    ...
    UI -- "HTTP/REST<br/>(팀·일정 CRUD, 채팅이력 조회)" --> Entry
    UI -- "WebSocket<br/>(실시간 채팅 송수신)" --> Entry
    Entry --> Logic
    Logic --> Data
    Data --> DB
```
...
- **백엔드 진입점**: HTTP 라우트와 WebSocket 게이트웨이를 아우르는 요청 수신 지점. 인증(UC1)은 이 지점에서 우회 불가능하게 강제된다(HTTP 미들웨어 / WebSocket 핸드셰이크).
...
- **HTTP/REST 연결**: 팀·일정·변경요청 CRUD 및 채팅 이력의 페이지네이션 조회(UC8)에 사용.
- **WebSocket 연결**: 캘린더-채팅 연동(UC5)의 실시간 메시지 송수신 전용 채널.
